  ...
  <div class="navbar bg-base-100 shadow-lg text-base py-2 sm:py-3 px-3 sm:px-4">
    <div class="flex-1">
      <NuxtLink to="/" class="flex items-center hover:opacity-80 transition-opacity duration-200">
        <img src="@/assets/logo-main.svg" alt="Toolhunt Logo" class="w-9 h-9 sm:w-11 sm:h-11 mr-2 sm:mr-3" >
    ...
    <div class="flex-none">
      <ul class="menu menu-horizontal px-1 hidden lg:flex">
        <li v-for="item in menuItems" :key="item.to">
          <NuxtLink :to="item.to" class="btn btn-ghost text-base sm:text-lg">{{ item.label }}</NuxtLink>
        </li>
      </ul>
      <a
class="btn btn-md ml-3 hidden lg:flex text-base sm:text-lg" :class="isLoggedIn ? 'bg-transparent border-primary/70 text-primary hover:bg-primary/10 hover:border-primary' : 'btn-primary'"
      ...
      <div class="dropdown dropdown-end lg:hidden ml-auto">
        ...
        <ul tabindex="0" class="menu menu-md dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
          <li v-for="item in menuItems" :key="item.to">
            <NuxtLink :to="item.to" class="text-base sm:text-lg">{{ item.label }}</NuxtLink>
          </li>
          <li>
            <a class="text-primary text-base sm:text-lg" @click="handleAuthAction">
              {{ isLoggedIn ? 'Logout' : 'Login' }}
            </a>
          </li>
        </ul>
